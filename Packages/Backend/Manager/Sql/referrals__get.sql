select
    ...
    coalesce(Referrals_count__last_week.referrals_count, 0) as `referrals_count__last_week`,
...
from
    ...
    ) as `Referrals_count__last_week` on Referrals_count__last_week.referral = Referrals.referral_tg_id
    left join (
        select
            Referrals.host_tg_id as `referral`,
            count(Referrals.host_tg_id) as `referrals_count`
